...
if (booleanComprobarNombre && booleanComprobarPrimerApellido && booleanComprobarSegundoApellido && booleanComprobarEmpleo && booleanComprobarNIF &&  booleanComprobarCorreoElectronico && booleanComprobarSeleccionIdioma && booleanComprobarCiudad && booleanComprobarDia && booleanComprobarMes) {
    ...
    //app.alert("verificacion correcta. \nA continuación, se procederá a la impresión del documento para que sea anexado esta solicitud. \nEl presente Documento será bloqueado, por lo que no se podrá modificar, para modificar el documento pulse el botón EDITAR ", 3);
    ...
    this.getField("NO PRECISA DE FIRMA EN ESTE DOCUMENTO").display = display.visible;
    
    // Muestra una advertencia indicando que se va a imprimir el documento
        if (app.alert("Se va a imprimir el documento. \nEl presente Documento será bloqueado, por lo que no se podrá modificar, para modificar o volver a imprimir el documento pulse el botón EDITAR. \n ¿Desea continuar?." , 1, 2) == 4) {
        // El usuario ha aceptado imprimir el documento
        this.print();
        ...
        // El usuario ha cancelado la impresión
        // Desbloquear y hacer visibles los campos y botones correspondientes
        this.getField("BORRAR DATOS FORMULARIO").display = display.visible;
        this.getField("IMPRIMIR").display = display.visible;
        
        for (var i = 0; i < camposABloquear.length; i++) {
            this.getField(camposABloquear[i]).readonly = false;
        }
